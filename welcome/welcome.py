		...
		for channel in self.channels:
			try:
				if self.channels[channel] and server.get_channel(channel).server == server:
					await self.bot.send_message(server.get_channel(channel), self.messages[action].format(("{} ({})".format(member.name, member.id), member.mention)[action == "joined"], server, server.member_count))
